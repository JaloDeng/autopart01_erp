-- *****************************************************************************************************
-- 创建存储过程 p_vendi_stock_by_bill, 以单据备货存储过程
-- *****************************************************************************************************
...
BEGIN
	...
	ELSEIF EXISTS(SELECT 1 FROM erp_sales_detail sd INNER JOIN ers_packageattr p ON p.id = sd.ers_packageAttr_id WHERE sd.erp_vendi_bil_id = vid AND p.degree > 1 LIMIT 1) THEN
		SET msg = concat('销售单（编号：', vid,'）存在配件是多层包装，不能完成整单备货！');
		SIGNAL SQLSTATE 'QZ000' SET MESSAGE_TEXT = msg;
	...
	-- 判断是否操作成功
	IF ROW_COUNT() = 0 THEN
		SIGNAL SQLSTATE 'QZ000' SET MESSAGE_TEXT = '整单备货时，写入备货失败！';
	END IF;
	...
	-- 判断是否操作成功
	IF ROW_COUNT() = 0 THEN
		SIGNAL SQLSTATE 'QZ000' SET MESSAGE_TEXT = '整单备货时，更新二维码表状态失败！';
	END IF;
	...
	SELECT vid, 'stock', uid, aid, aName, aUserName, '整单备货';
